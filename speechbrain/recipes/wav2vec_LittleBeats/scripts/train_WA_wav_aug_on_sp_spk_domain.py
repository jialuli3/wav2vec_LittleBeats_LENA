...
class EmoIdBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        wavs, lens = batch.sig
        wavs_sp, lens_sp = batch.sig
        domain_labels = batch.domain_labels
        domain_labels_sp = batch.domain_labels
        ...
        if stage == sb.Stage.TRAIN:
            ...
            for count, augment in enumerate(self.hparams.augment_pipeline):
                ...
                if self.hparams.concat_augment:
                    ...
                    wavs_sp = wavs_aug
                    wavs_aug_tot[0] = wavs_sp

            wavs_sp = torch.cat(wavs_aug_tot, dim=0)
            self.n_augment = len(wavs_aug_tot)
            lens_sp = torch.cat([lens] * self.n_augment)
            domain_labels_sp = torch.cat([domain_labels]*self.n_augment)

        outputs_sp = self.modules.wav2vec2(wavs_sp)
        outputs_voc = self.modules.wav2vec2(wavs)
        
        # last dim will be used for AdaptativeAVG pool
        spk_embs = self.compute_embeddings(wavs, lens)

        outputs_sp = self.get_wa_outputs(outputs_sp,self.modules.weighted_average_sp,self.modules.dnn_sp,self.modules.output_mlp_sp, \
                domain_labels_sp,None,mean_pool_first=self.hparams.mean_pool_first_sp, domain_concat_first=self.hparams.domain_concat_first, lens = lens_sp)
        outputs_chn = self.get_wa_outputs(outputs_voc,self.modules.weighted_average_chn,self.modules.dnn_chn,self.modules.output_mlp_chn, \
                domain_labels,spk_embs, mean_pool_first=self.hparams.mean_pool_first_chn,  domain_concat_first=self.hparams.domain_concat_first, lens = lens)
        outputs_adu = self.get_wa_outputs(outputs_voc,self.modules.weighted_average_adu,self.modules.dnn_adu,self.modules.output_mlp_adu, \
                domain_labels,spk_embs, mean_pool_first=self.hparams.mean_pool_first_adu, domain_concat_first=self.hparams.domain_concat_first, lens = lens)


        return outputs_sp, outputs_chn, outputs_adu

    def get_wa_outputs(self, outputs, wa, dnn, mlp, domain_labels, spk_embs, mean_pool_first=True, domain_concat_first=False, lens=None):
        domain_embs = self.modules.domain_emb(domain_labels)
        if mean_pool_first:
            avg_outputs = []
            for i in range(len(outputs)):
                avg_output = self.hparams.avg_pool(outputs[i],lens)
                avg_output = avg_output.view(avg_output.shape[0],-1)
                if domain_concat_first:
                    avg_output = torch.cat((avg_output,domain_embs),dim=-1)
        ...
        if not domain_concat_first:
            outputs = torch.cat((outputs,domain_embs),dim=-1)

        if spk_embs is not None:
            outputs = torch.cat((outputs,spk_embs),dim=-1)
    ...
    def compute_objectives(self, outputs_sp, outputs_chn, outputs_adu, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            sp_true = torch.cat([sp_true] * self.n_augment, dim=0)
    ...
    def compute_embeddings(self, wavs, wav_lens):
        ...
        with torch.no_grad():
            ...
            embeddings = self.hparams.embedding_model(feats, wav_lens)
...
def dataio_prep(hparams):
    ...
    @sb.utils.data_pipeline.takes("domain_label")
    @sb.utils.data_pipeline.provides("domain_labels")
    def label_pipeline_domain(input):
        dict_map={"LB":0,"LENA":1,"IDP_LAB":1}
    ...
    for dataset in ["train", "valid", "test"]:
        datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_json(
            ...
            dynamic_items=[audio_pipeline, label_pipeline_sp, label_pipeline_chn, label_pipeline_adu,label_pipeline_domain],
            output_keys=["id", "sig", "sp_true", "chn_true", "adu_true","domain_labels"],
        )
